(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('chatAnalysisApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-chatAnalysisApp-error');
                var entityKey = httpResponse.headers('X-chatAnalysisApp-params');
                if (errorHeader) {
